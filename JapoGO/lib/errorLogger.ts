...
export function logError(
  ...
  try {
    const savedErrors = localStorage.getItem('JapoGo_error_logs');
    ...
    localStorage.setItem('JapoGo_error_logs', JSON.stringify(trimmedLogs));
  } catch (storageError) {
...
export function clearErrorLogs(): void {
  try {
    localStorage.removeItem('JapoGo_error_logs');
  } catch (error) {
